 ...
 */
/*
 * Original copyrighted file link: http://www.mcs.anl.gov/petsc/petsc-3.4/src/snes/examples/tutorials/ex14.c.html
 */

/* 
 * File       : AppWrap.c
 * Description: contains wrapper routines that are able to communicate from
 *              C side to Petsc C side
 * Author     : Mohsin Ali
...
//=================================================================================
//=================================================================================
PetscErrorCode c_runApp(char * in_dir, int proc_x, int proc_y, int proc_z, MPI_Comm c_comm, 
              bool comp_g, bool rep_comb, int n, int nx, int ny, int nz, int n_comb, 
              bool real_call) {
...
//Mohsin added this
  ...
  PetscInt my_dims = 3, my_index = 0;
  PetscScalar ***x_array_3d;
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     ...
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ierr = SNESCreate(c_comm,&snes);CHKERRQ(ierr);

  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     Create distributed array (DMDA) to manage parallel grid and vectors
  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ierr = DMDACreate3d(c_comm,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DMDA_STENCIL_STAR,nx,ny,nz,proc_x,proc_y,
                      proc_z,1,1,NULL,NULL,NULL,&user.da);CHKERRQ(ierr);
...
//Mohsin added this
  ...
  ierr = MPI_Comm_rank(c_comm, &my_rank);CHKERRQ(ierr);
  ierr =  DMDAVecGetArray(user.da, x, &x_array_3d);CHKERRQ(ierr);
  ierr = DMDAGetCorners(user.da,&x_start,&y_start,&z_start,&x_width,&y_width,&z_width);CHKERRQ(ierr);

  PetscScalar array_1d_to_1d[x_width];
  PetscScalar array_2d_to_1d[x_width*y_width];
  PetscScalar array_3d_to_1d[x_width*y_width*z_width];

  for (k=z_start; k<z_start+z_width; k++) {
    for (j=y_start; j<y_start+y_width; j++) {
      for (i=x_start; i<x_start+x_width; i++) {
          //printf("[%d] val = %0.6lf\n", my_rank, x_array_3d[k][j][i]);
          array_3d_to_1d[my_index] = x_array_3d[k][j][i];
          my_index++;
      }
    }
  }
  ierr =  DMDAVecRestoreArray(user.da, x, &x_array_3d);CHKERRQ(ierr);
  if(real_call) {
     c_get_app_field(&my_dims, array_1d_to_1d, array_2d_to_1d, array_3d_to_1d, &x_start, &y_start, &z_start,
          &x_width, &y_width, &z_width, &comp_g, &rep_comb);
  }
  ...
  ierr = PetscPrintf(c_comm,"Number of SNES iterations = %D fnorm %g\n",its,(double)fnorm);CHKERRQ(ierr);
